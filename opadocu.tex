...
\parbox[t]{0.7\hsize}{
  {\Huge\bf OPA documentation} version {\bf 4.3.0}\\
  Andreas Streun, \today
...
In version 4.3.0 OPA includes 48 Pascal units \code{.pas}, 29 of them are \opagui{GUIs} which are accompanied by a Lazarus form \code{.lfm} (created by the Lazarus IDE), and 19 of them are Pascal \opauni{units}. Four of the GUIs are \opaguif{frames}, i.e. self-defined components embedded in other GUIs.
...
Procedures are short and probably self-explaining. Not all of them are used in OPA (the \code{com*} files may be used by other programs too.}
...
\desc{This unit contains the procedures to propagate optical parameters through the different elements in the lattice. They all receive the variable \unico{mode} telling them what to include in the calculation, by evaluating \prcod{elemlib}{switch}. Most elements receive the varialbe \unico{idir} to set forward or reverse orientation of the element (for example important for a dipole with different edge angles). All other parameters are explained in the user guide \cite{userguide}. 
...
\act{}
\evcodx{arrow buttons}{but(Left,Right,Up,Down,SForward,SBackward,ZoomFull)Click} select the view by shifting, by following the lattice or by returning to full view. Further marking a rectangle with the mouse selects a region of interest (see \oguif{comfigureframe}, \todo{BUG: does not work in the linux version}).
...
\evcod{$\longrightarrow$Data}{butListClick} calls \prcod{georblib}{WriteGEoFiles} to export text files with geometry data.
...
\unico{WriteGeoFiles} exports file \code{*\_geo.txt} of polygon data. \todo{Further files of geometry data named \code{*\_(holy,srad,mpo,pos).txt} are exported too, however these have only historical or SLS-specific meanings and may be adapted as needed.}
